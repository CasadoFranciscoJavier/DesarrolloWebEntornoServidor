...
### server.php se borra constantemente
- **Causa:** Windows Defender / Antivirus detecta server.php como amenaza y lo borra
- **Solución DEFINITIVA:** Crear `server.php` en la RAÍZ del proyecto (no en vendor)
  - Laravel busca primero en la raíz, luego en vendor
  - El de la raíz NO lo toca el antivirus
- **Contenido del archivo:**
  ...
  ```
- **Ubicación:** `UD3/holaMundo/server.php` (raíz del proyecto Laravel)
- **Alternativa:** Añadir exclusión en Windows Defender para la carpeta del proyecto

### Error 404 en formularios POST
- **Problema común:** La ruta del `action` en el formulario no coincide con la ruta definida en `web.php`
- **Verificar:** Que `<form action="/ruta">` coincida exactamente con `Route::post('/ruta', ...)`
- **Ejemplo error:** Formulario con `action="/usuario"` pero ruta POST en `/nuevo_usuario`
