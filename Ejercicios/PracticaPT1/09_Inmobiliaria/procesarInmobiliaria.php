<?php
require_once "bbddInmobiliaria.php";

function alert($text)
{
    echo "<script> alert('$text');window.location.href = 'inmobiliaria.html'; </script>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $todoValido = validarString("nombre", 2, 30)
        ...
        && validarSeleccion("propiedad")
        && validarSeleccion("ubicacion")
    ...
    if ($todoValido) {
        ...
        $duracionSeleccionada = $_POST["duracion"];

        $codigoPromoIngresado = isset($_POST["codigo"]) ? $_POST["codigo"] : "";
        $serviciosSeleccionados = isset($_POST["servicios"]) ? $_POST["servicios"] : [];

        // Calcular precios
        $precioBasePropiedad = $propiedades[$propiedad];
        $multiplicadorUbicacion = $ubicaciones[$ubicacion];
        $multiplicadorDuracion = $duracion[$duracionSeleccionada];
        $precioPropiedad = $precioBasePropiedad * $multiplicadorUbicacion * $multiplicadorDuracion;
        ...
        // Calcular totales
        $subtotal = $precioPropiedad + $precioServicios;
...
<head>
    ...
    <title>Resumen de Alquiler - Inmobiliaria</title>
</head>

<body>
    <div class="contenedor">
        <h1>INMOBILIARIA - RESUMEN DE ALQUILER</h1>
        ...
        <?php if ($_SERVER["REQUEST_METHOD"] == "POST" && $todoValido): ?>
            ...
            <p><strong>Tipo:</strong> <?php echo htmlspecialchars($propiedad); ?></p>
            <p><strong>Ubicación:</strong> <?php echo htmlspecialchars($ubicacion); ?></p>
            <p><strong>Duración:</strong> <?php echo htmlspecialchars($duracionSeleccionada); ?></p>
            <p><strong>Coste propiedad:</strong> <?php echo number_format($precioPropiedad, 2); ?> ¬ (<?php echo $precioBasePropiedad; ?> ¬ × <?php echo $multiplicadorUbicacion; ?> × <?php echo $multiplicadorDuracion; ?>)</p>
            ...
            <ul>
                <li><strong>Propiedad:</strong> <?php echo number_format($precioPropiedad, 2); ?> ¬</li>
